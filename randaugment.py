...
def autocontrast_func(img, cutoff=0):
    ...
    def tune_channel(ch):
        ...
        if high <= low:
            ...
            scale = min((n_bins - 1) / max(high - low, 1e-5), 1e5)  # Stricter cap on scale
            offset = max(-low * scale, -1e5)  # Stricter cap for offset
            #print(f"Scale: {scale}, Offset: {offset}, High: {high}, Low: {low}")  # Debugging line
